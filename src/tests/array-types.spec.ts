...
describe('array types', () => {
  ...
  it('casts items with types.array(types.number)', async () => {
    const parser = createParser({ values: types.array(types.number) });
  ...
  it('supports nested item types', async () => {
    const parser = createParser({ matrix: types.array(types.array(types.number)) });
  ...
  it('applies looseCasting per item', async () => {
    const parser = createParser({ values: types.array(types.number) }, { looseCasting: 'undefined' });
  ...
  it('keeps parameterized tokens identity-stable', () => {
    expect(types.array(types.number)).toBe(types.array(types.number));
  });

  it('throws when calling a non-array type', () => {
    expect(() => (types.string as unknown as (arg: unknown) => unknown)(types.number)).toThrow('does not accept arguments');
    expect(() => (types.array as unknown as (arg: unknown) => unknown)('number')).toThrow('expects a type');
  });
});
